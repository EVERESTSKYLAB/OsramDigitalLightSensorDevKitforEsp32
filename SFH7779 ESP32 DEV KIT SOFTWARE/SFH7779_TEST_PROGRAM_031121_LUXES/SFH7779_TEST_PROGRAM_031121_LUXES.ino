...
//On the ESP32 DEV KIT C4 can find 2 ports i2c recommended, for SFH7779 will be used i2c port 1; SCL_SENS_pin1 = 33, SDA_SENS_pin1 = 32
Basic Program:
Initialization data
Set up ESP32 port pin, speed and clock
...
void Read_Sensors() 
{
   ...
   Vis_Value = 256*(32768*bitRead(Vis_MSB,7) + 16386*bitRead(Vis_MSB,6) + 8192*bitRead(Vis_MSB,5) + 4096*bitRead(Vis_MSB,4) + 2048*bitRead(Vis_MSB,3) + 1024*bitRead(Vis_MSB,2) + 512*bitRead(Vis_MSB,1) + 256*bitRead(Vis_MSB,0) )+ 128*bitRead(Vis_LSB,7)+ 64*bitRead(Vis_LSB,6)+ 32*bitRead(Vis_LSB,5)+ 16*bitRead(Vis_LSB,4)+ 8*bitRead(Vis_LSB,3)+ 4*bitRead(Vis_LSB,2) + 2*bitRead(Vis_LSB,1)+ 1*bitRead(Vis_LSB,0);
   ...
   //The two ambient light sensors deliver output values in the range from 0 to 65535 (16 bit).
   IR_Value = 256*(32768*bitRead(IR_MSB,7) + 16386*bitRead(IR_MSB,6) + 8192*bitRead(IR_MSB,5) + 4096*bitRead(IR_MSB,4) + 2048*bitRead(IR_MSB,3) + 1024*bitRead(IR_MSB,2) + 512*bitRead(IR_MSB,1) + 256*bitRead(IR_MSB,0)) + 128*bitRead(IR_LSB,7)+ 64*bitRead(IR_LSB,6)+ 32*bitRead(IR_LSB,5)+ 16*bitRead(IR_LSB,4)+ 8*bitRead(IR_LSB,3)+ 4*bitRead(IR_LSB,2) + 2*bitRead(IR_LSB,1)+ 1*bitRead(IR_LSB,0);
   Serial.print ("   IR=  ");
...
// Lux value in front of sensor, no cover glass
    ...
    //The following equations are recommended to be applied to calculate the true ALS lux-value out of the ALS_VIS and ALS_IR data. The equations are valid for
    //the illumination in front of the sensor (e.g. no cover glass or glasses with flat transmission characteristics from visible into the IR region). For applications with
    //a (dark) cover glass please refer to Section I.App Note No.: AN100
    if (REF<0.109){
...
//After activation, the sensor will change from STAND-BY to FREERUNNING mode. After a delay of e.g. 100 ms (depending on t_INT_ALS
