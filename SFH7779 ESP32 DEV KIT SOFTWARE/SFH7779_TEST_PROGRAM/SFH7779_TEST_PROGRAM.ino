//SKYLAB MARCH 4th, 2021
//Roberto Gonzalez by Everest Skylab
...
const int SCLpin = 33;          // declares pin 33 as Clock pin
const int SDApin = 32;          // declares pin 32 as Data pin
const int INT_Pin = 14;         // declares the Pin 14 as an interrupt INT_Pin

byte Prox_M;        //  declares BYTE MSB of Proximity Measurement Data
byte Prox_l;        //  declares BYTE LSB of Proximity Measurement Data
byte Vis_M;         //  declares BYTE MSB of Ambient Light Measurement of VIS diode
byte Vis_l;         //  declares BYTE LSB of Ambient Light Measurement of VIS diode
byte IR_M;          //  declares BYTE MSB of Ambient light measurement of IR diode
byte IR_l;          //  declares BYTE LSB of Ambient light measurement of IR diode
int Prox_Val;       //  declares BYTE FOR Total VALUE OF Proximity Measurement Data
int Vis_Val;        //  declares BYTE FOR Total VALUE OF Ambient Light Measurement of VIS diode
int IR_Val;         //  declares BYTE FOR Total VALUE OF Ambient light measurement of IR diode
...
void Read_Sensors() 
{
 ...
 //Initialization of Proximity sensor, VIS ALS and IR ALS
   Prox_M = 0;      //  SET to "0" BYTE MSB of Proximity Measurement Data
   Prox_l = 0;      //  SET to "0" BYTE LSB of Proximity Measurement Data
   Vis_M = 0;       //  SET to "0" BYTE MSB of Ambient Light Measurement of VIS diode
   Vis_l = 0;       //  SET to "0" BYTE LSB of Ambient Light Measurement of VIS diode
   IR_M = 0;        //  SET to "0" BYTE MSB of Ambient light measurement of IR diode
   IR_l = 0;        //  SET to "0" BYTE LSB of Ambient light measurement of IR diode
   ...
   Prox_l = Wire.read();            //PS_DATA_LSBs register (0x44h) LSB of the PS output measurement 
   //Serial.print(0x39, Prox_l);  
   Prox_M = Wire.read();            //PS_DATA_MSBs register (0x45h) MSB of the PS output measurement        
   //Serial.print(Prox_M)5
   ...
   Vis_l = Wire.read();             //ALS_VIS_DATA_LSBs register (0x46h) LSB of the ALS VIS output measurement 
   //Serial.print(Vis_l);
   Vis_M = Wire.read();             //ALS_VIS_DATA_MSBs register (0x47h) MSB of the ALS VIS output measurement 
   //Serial.print(Vis_M);
   IR_l = Wire.read();              //ALS_IR_DATA_LSBs register (0x48h) LSB of the ALS IR output measurement
   //Serial.print(IR_l);
   IR_M = Wire.read();              //ALS_IR_DATA_MSBs register (0x49h) MSB of the ALS IR output measurement
   //Serial.print(IR_M);
   ...
   Prox_l = 128*bitRead(Prox_l,7)+ 64*bitRead(Prox_l,6)+ 32*bitRead(Prox_l,5)+ 16*bitRead(Prox_l,4)+ 8*bitRead(Prox_l,3)+ 4*bitRead(Prox_l,2) + 2*bitRead(Prox_l,1)+ 1*bitRead(Prox_l,0);
   Serial.print ("   Prox_l=  ");
   Serial.print(Prox_l);
   Prox_M = 2048*bitRead(Prox_M,3) + 1024*bitRead(Prox_M,2) + 512*bitRead(Prox_M,1) + 256*bitRead(Prox_M,0);
   Serial.print ("   Prox_M=  ");
   Serial.print(Prox_M);
   Prox_Val = 2048*bitRead(Prox_M,3) + 1024*bitRead(Prox_M,2) + 512*bitRead(Prox_M,1) + 256*bitRead(Prox_M,0) + 128*bitRead(Prox_l,7)+ 64*bitRead(Prox_l,6)+ 32*bitRead(Prox_l,5)+ 16*bitRead(Prox_l,4)+ 8*bitRead(Prox_l,3)+ 4*bitRead(Prox_l,2) + 2*bitRead(Prox_l,1)+ 1*bitRead(Prox_l,0);
...
void setup()
{
  ...
  //SFH7779 Designed for the I2C Fast mode (400 kb/s) 
  Serial.begin(9600);
...
// SET ALS_PS_CONTROL REGISTER AND ACTIVATE ALS AND PS MODE_CONTROL REGISTER 
    ...
    //SET PROXIMITY LED CURRENT TO 200MA AND ALS GAIN TO 128
          ...
          Wire.write(0x3F); //Write 0x3F (command) to set LED pulse current to 200mA and ALS GAIN to x128
    ...
    //ACTIVATE ALS (INT AND REP TIME) AND PS (REP TIME)
          ...
          Wire.write(0x06);                 //Write 0x06 (command) to Activate ALS & PS with a ALS interuption/repetition time=100ms and PS repetition time=100ms 
          Wire.endTransmission();
}
